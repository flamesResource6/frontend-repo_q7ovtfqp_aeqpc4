...
function VisualPYQ() {
  return (
    <div className="relative h-10 w-10 rounded-xl bg-sky-50 ring-1 ring-white/60 shadow shadow-sky-200/70 grid place-items-center overflow-hidden">
      {/* Paper stack */}
      <div className="absolute inset-0 scale-110">
        <div className="absolute left-1 top-1 h-7 w-9 rotate-[-2deg] rounded-md bg-white/90 ring-1 ring-slate-200" />
        <div className="absolute left-2 top-2 h-7 w-9 rotate-[2deg] rounded-md bg-white/90 ring-1 ring-slate-200" />
      </div>
...
function VisualAnalytics() {
  const heights = [18, 24, 14, 28, 20];
  return (
    <div className="relative h-10 w-10 rounded-xl bg-emerald-50 ring-1 ring-white/60 shadow shadow-emerald-200/70 px-1.5 pb-1.5 pt-2">
...
function VisualMentor() {
  ...
  return (
    <div className="relative h-10 w-10 rounded-xl bg-cyan-50 ring-1 ring-white/60 shadow shadow-cyan-200/70">
      <motion.div
      ...
      />
      <div className="absolute bottom-1 left-1 h-4 w-4 rounded-full bg-sky-400 ring-2 ring-white shadow" />
      <div className="absolute top-1.5 right-1.5 h-4 w-4 rounded-full bg-emerald-400 ring-2 ring-white shadow" />
      {dots.map((p, i) => (
        <span key={i} className="absolute h-1 w-1 rounded-full bg-cyan-400/60" style={{ left: p.x, top: p.y, opacity: 0.7 - i * 0.1 }} />
      ))}
      <motion.span
        className="absolute h-1.5 w-1.5 rounded-full bg-cyan-500 shadow"
        initial={{ x: 4, y: 20 }}
        animate={{ x: [4, 9, 14, 19, 24], y: [20, 16, 12, 10, 7] }}
        transition={{ duration: 2.2, repeat: Infinity, repeatType: "mirror", ease: "easeInOut" }}
      />
      <motion.div
        className="absolute right-1.5 top-5 flex flex-col gap-0.5"
        initial="hidden"
        animate="show"
        variants={{
          hidden: {},
...
function VisualBoardsEntrance() {
  return (
    <div className="relative h-10 w-10 rounded-xl bg-indigo-50 ring-1 ring-white/60 shadow shadow-indigo-200/70 p-1.5">
      <div className="text-[7px] text-slate-600">Boards</div>
      ...
      <div className="mt-1 text-[7px] text-slate-600">Entrance</div>
      ...
      <motion.div className="absolute right-1.5 top-1.5 h-1.5 w-1.5 rounded-full bg-indigo-500" animate={{ y: [0, -2, 0] }} transition={{ duration: 1.4, repeat: Infinity }} />
    </div>
  );
}
...
function VisualPredictor() {
  return (
    <div className="relative h-10 w-10 rounded-xl bg-amber-50 ring-1 ring-white/60 shadow shadow-amber-200/70 p-1.5 overflow-hidden">
      ...
      <motion.div className="absolute -bottom-0.5 h-2.5 w-2.5 rounded-full bg-amber-500 shadow" animate={{ x: [2, 30, 18, 36, 10, 26, 6] }} transition={{ duration: 3.2, repeat: Infinity, ease: "easeInOut" }} />
      <div className="absolute top-0.5 right-0.5 text-[7px] text-amber-700/80 bg-white/70 rounded px-0.5">Predict</div>
    </div>
  );
}
...
function VisualPlanner() {
  return (
    <div className="relative h-10 w-10 rounded-xl bg-fuchsia-50 ring-1 ring-white/60 shadow shadow-fuchsia-200/70 p-1.5">
...
export default function Features() {
  ...
  return (
    <section className="relative py-12 sm:py-16 overflow-hidden">
      ...
      <div
        aria-hidden
        className="absolute -top-24 right-1/2 h-72 w-[120%] -translate-x-1/3 rotate-[-8deg] blur-2xl opacity-[0.32] -z-20"
        style={{
          backgroundImage:
      ...
      <div
        aria-hidden
        className="absolute -bottom-36 left-1/2 h-80 w-[120%] -translate-x-1/2 rotate-[10deg] blur-2xl opacity-[0.26] -z-20"
        style={{
          backgroundImage:
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          <motion.h2 variants={fadeUp} className="text-2xl sm:text-3xl font-extrabold tracking-tight text-slate-900">
          ...
          <motion.p variants={fadeUp} className="mt-2 text-slate-600 text-[15px]">
            Built for Classes 11–12 and droppers preparing for JEE Main, Advanced, BITSAT, EAMCET, MHT CET and Boards.
          </motion.p>
          <motion.div
            variants={fadeUp}
            className="mx-auto mt-3 h-[5px] w-24 rounded-full bg-gradient-to-r from-sky-400 to-emerald-400 shadow-[0_0_14px_rgba(16,185,129,0.24)]"
          />
        </motion.div>
          ...
          {items.map((it, idx) => {
            ...
            return (
              <motion.div
                ...
                variants={fadeUp}
                whileHover={{ y: -2 }}
                className="group relative rounded-2xl bg-white ring-1 ring-slate-100 p-4 shadow-sm transition-all duration-300 hover:shadow-md"
              >
                <div
                ...
                <h3 className="mt-3 font-semibold text-slate-900 text-[15px]">
                  {it.title}
                </h3>
        ...
        >
          <div className="relative inline-flex items-center gap-2.5 rounded-2xl bg-white/80 px-3.5 py-2 ring-1 ring-slate-200 shadow-sm">
            <span className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" />
            <span className="text-[13px] text-slate-700">New features land every week • Roadmap driven by students</span>
          </div>
        </motion.div>
      </div>
    </section>
